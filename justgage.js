...
JustGage = function(config) {
    ...
    obj.config =
    {
        ...
        valueFontSetSize : config.valueFontSetSize || 12,
    ...
    if (obj.config.relativeGaugeSize === true) {
        ...
        if (obj.config.vertical === true) {
            var divW = getStyle(document.getElementById(obj.config.id), "width").slice(0, -2) * 1;
            var divH = getStyle(document.getElementById(obj.config.id), "height").slice(0, -2) * 1;
    ...
    if (obj.config.donut === true) {
        ...
        if (obj.config.vertical === true) {
            ...
            var width = getStyle(document.getElementById(obj.config.id), "width").slice(0, -2) * 1;
            ...
            valueFontSize = ((widgetH / 6.5) < obj.config.valueFontSetSize) ? (widgetH / 6.5) : obj.config.valueFontSetSize;
            valueX = (dx * 2) + (width / 2);
            valueY = dy + widgetH / 1.5;
    ...
    obj.canvas.customAttributes.pki = function (value, min, max, w, h, dx, dy, gws, donut, vertical) {
        ...
        if (vertical) {
            ...
            path += "M" + (width / 2) + " " + (h);   // Move to
            path += "H" + ((width / 2) + (Ro * 0.35));   // Horizontal line to
            path += "V" + (h - ( (h * (0.74 / max)) * value));   // Vertical line to
            path += "H" + (width / 2);   // Horizontal line to
            path += "V" + (h);   // Vertical line to
            path += "Z ";    // Closepath
